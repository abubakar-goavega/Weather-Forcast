...
function setLoader(loader: Element, loadingState: LoadingState): void {
...
function formateDecimal(num: number): string | number {
    return num >= 10 ? num : '0' + num;
}

function formateTime(date = new Date()): [String, String] {
...
async function getLatLonByCityName(cityName: string) {
 ...
 * 8 * 3 = 24
 */
function filterFiveDaysWeatherForcast(list: APIResponseList[]): APIResponseList[] {
...
function formateApiDate(dateTxt: string): Date {
    const [year, month, date] = dateTxt.split(' ')[0].split('-');
    return new Date(parseInt(year), parseInt(month) - 1, parseInt(date));
}

function formateDay(date: Date): string {
...
function addFutureForecast(forcasteEle: HTMLElement, foreCast: APIResponseList[]) {
...
function setCityDetails(country: string, name: string, timezone: number, coord: { lat: number, lon: number }) {
...
async function setWeatherData(lat = -1, lon = -1): Promise<void> {
...
if (searchBtn !== null)
    searchBtn.addEventListener('click', () => {
        try {
            if (searchInput !== null)
                getLatLonByCityName((searchInput as HTMLInputElement).value).then(obj => {
